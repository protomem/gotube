...
func (repo *VideoRepository) FindByAuthorNicknameSortByCreatedAt(
	...
	const op = "postgres.VideoRepository.FindByAuthorNicknameSortByCreatedAt"

	query := `
		SELECT videos.*, authors.*
		FROM videos
		JOIN users AS authors ON videos.author_id = authors.id
		WHERE authors.nickname = $1
		ORDER BY videos.created_at DESC
	`

	rows, err := repo.db.QueryContext(ctx, query, nickname)
	if err != nil {
		if pgerr.IsNotFound(err) {
			return []model.Video{}, nil
		}
...
func (repo *VideoRepository) FindPublicByLikeTitleSortByCreatedAt(
	ctx context.Context,
	titleQuery string,
	opts repository.FindVideosOptions,
) ([]model.Video, error) {
	...
	query := `
		...
		FROM videos
		JOIN users AS authors ON videos.author_id = authors.id
		WHERE videos.is_public = true AND lower(videos.title) LIKE '%' || lower($3) || '%'
		ORDER BY videos.created_at DESC
		LIMIT $1
		OFFSET $2
	`

	rows, err := repo.db.QueryContext(ctx, query, opts.Limit, opts.Offset, titleQuery)
	if err != nil {
		if pgerr.IsNotFound(err) {
			return []model.Video{}, nil
		}
